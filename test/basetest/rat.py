...
from .utils import run_cmd_wait, run_cmd_wait_nofail, which, rat_binary_location, DEFAULT_EXTENSION_PATH
from .compat import STRING_TYPE


class Rat(object):
    """Manage a Rat instance

    A temporary folder is used as data store of rat.

    A rat client should not be used after being destroyed.
    """
    DEFAULT_RAT = rat_binary_location()

    def __init__(self, rat=DEFAULT_RAT):
        """Initialize a rat (client).
        The program runs in a temporary folder.

        :arg rat: Rat binary to use as client (defaults: rat in PATH)
        """
        self.rat = rat

        # Used to specify what command to launch (and to inject faketime)
        self._command = [self.rat]

        # Configuration of the isolated environment
        self._original_pwd = os.getcwd()
        self.datadir = tempfile.mkdtemp(prefix="rat_")
    ...
    def __repr__(self):
        txt = super(Rat, self).__repr__()
        return "{0} running from {1}>".format(txt[:-1], self.datadir)

    def __call__(self, *args, **kwargs):
        "aka t = Rat() ; t() which is now an alias to t.runSuccess()"
    ...
    def runSuccess(self, args="", input=None, merge_streams=False,
                   timeout=5):
        """Invoke rat with given arguments and fail if exit code != 0

        Use runError if you want exit_code to be tested automatically and
        *not* fail if program finishes abnormally.

        If you wish to pass instructions to rat such as confirmations or other
        ...
        timeout = number of seconds the test will wait for every rat call.
