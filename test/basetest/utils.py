...
# Environment flags to control skipping of rat tests
RAT_SKIP = os.environ.get("RAT_SKIP", False)
# Environment flags to control use of PATH or in-tree binaries
RAT_USE_PATH = os.environ.get("RAT_USE_PATH", False)

UUID_REGEXP = ("[0-9A-Fa-f]{8}-" + ("[0-9A-Fa-f]{4}-" * 3) + "[0-9A-Fa-f]{12}")


def rat_binary_location(cmd="rat"):
    """ ../src/ is used by default.
    """
    return os.path.join(BIN_PREFIX, cmd)
    return binary_location(cmd, RAT_USE_PATH)
